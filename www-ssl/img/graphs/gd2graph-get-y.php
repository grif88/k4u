...
// vertical lines
if ( $hour < 8 ) { $temppix=0; }
else if ( $hour < 16 ) { $temppix=1; }
else { $temppix=2; }
$kolpix=(($day-1)*3)+$temppix+1;
$tit_mon=array(1=>'Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec');
$tmp_mon=$month;
$jj=0;
for ($i=591-$kolpix; $i>=90; $i-=90){
    imagedashedline($img1, $i, 10, $i, 160, $linecol2); // vertical line
    $xpos=$i+36;
    if ( $jj == 0 && $day < 18 ) {
	$jj=1;
    } else {
	imagestring($img1, 2, $xpos, 161, $tit_mon[$tmp_mon], $txtcol1); // day name
	$jj=1;
    }
    if ( $tmp_mon == 1 ) { $tmp_mon=12; } else { $tmp_mon--; }
}
