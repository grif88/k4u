...
if ( isset($_GET['mkt']) && !empty($_GET['mkt']) && isset($_GET['mib']) && !empty($_GET['mib']) ) {
	$mkt=$_GET['mkt'];
	$mib1=$_GET['mib'];
	#$r=exec('snmpget -v 2c -r 3 -t 1 -Oqv -c pub321lic123 '.$mkt_ip[$mkt].' .1.3.6.1.2.1.31.1.1.1.6.'.$mib1); // Rx trafic
	#$t=exec('snmpget -v 2c -r 3 -t 1 -Oqv -c pub321lic123 '.$mkt_ip[$mkt].' .1.3.6.1.2.1.31.1.1.1.10.'.$mib1);// Tx trafic
	exec('snmpget -v 2c -r 3 -t 1 -Oqv -c pub321lic123 '.$mkt_ip[$mkt].' .1.3.6.1.2.1.31.1.1.1.6.'.$mib1.' .1.3.6.1.2.1.31.1.1.1.10.'.$mib1.' .1.3.6.1.2.1.1.3.0',$snmpans); // full answer
	$r=$snmpans[0];
	$t=$snmpans[1];
	$uptimeA=explode(':',$snmpans[2]);
	$uptime=$uptimeA[0]*86400+$uptimeA[1]*3600+$uptimeA[2]*60+$uptimeA[3];
	$timeint=1;
	$res=mysql_query("SELECT `uptime`,`prevval` FROM `t_temp_load` WHERE `mkt`='$mkt' AND `type`='r' AND `user`='$php_user' ORDER BY `id` DESC LIMIT 0,1");
	...
	$rprev=$r/1024*8;
	$res=mysql_query("SELECT `uptime`,`prevval` FROM `t_temp_load` WHERE `mkt`='$mkt' AND `type`='t' AND `user`='$php_user' ORDER BY `id` DESC LIMIT 0,1");
	if ( mysql_num_rows($res) == 1 ) {
		while ( $tmp=mysql_fetch_assoc($res) ){
			$tprev=$tmp['prevval'];
			$prevuptime=$tmp['uptime'];
	...
	$tprev=$t/1024*8;
	if ( $timeint != 0 ) {
		mysql_query("INSERT INTO `t_temp_load` (`uptime`,`prevval`,`val`,`mkt`,`type`,`user`) VALUES ('$uptime','$rprev','$r2','$mkt','r','$php_user')");
		mysql_query("INSERT INTO `t_temp_load` (`uptime`,`prevval`,`val`,`mkt`,`type`,`user`) VALUES ('$uptime','$tprev','$t2','$mkt','t','$php_user')");
	}
	//----------------------------------------------
// picture
...
$linecol3 = imagecolorallocate($img1, 0, 255, 0); // r line
$linecol4 = imagecolorallocate($img1, 0, 0, 255); // t line
...
// select r
	$res2=mysql_query("SELECT `val` FROM `t_temp_load` WHERE `mkt`='$mkt' AND `type`='r' AND `user`='$php_user' ORDER BY `id` DESC LIMIT 0,501");
...
// select t
	$res3=mysql_query("SELECT `val` FROM `t_temp_load` WHERE `mkt`='$mkt' AND `type`='t' AND `user`='$php_user' ORDER BY `id` DESC LIMIT 0,501");
...
// comment
imagestring($img1, 3, 30, 168, "max = $max_z Kb/s", $txtcol2); // max
imagestring($img1, 3, 175, 168, "min = $min_z Kb/s", $txtcol3); // min
imagestring($img1, 3, 320, 168, "ave = $ave_z Kb/s", $txtcol4); // ave
imagestring($img1, 3, 465, 168, "cur = $cur_z Kb/s", $txtcol1); // cur
imagestring($img1, 3, 8, 168, 'Rx', $linecol3); // info
imagestring($img1, 3, 30, 180, "max = $max_z3 Kb/s", $txtcol2); // max 3
imagestring($img1, 3, 175, 180, "min = $min_z3 Kb/s", $txtcol3); // min 3
imagestring($img1, 3, 320, 180, "ave = $ave_z3 Kb/s", $txtcol4); // ave 3
imagestring($img1, 3, 465, 180, "cur = $cur_z3 Kb/s", $txtcol1); // cur 3
imagestring($img1, 3, 8, 180, 'Tx', $linecol4); // info 3
imagestringup($img1, 4, 12, 150, 'trafic per 2 sec', $txtcol1); // title
